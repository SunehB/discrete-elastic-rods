...
void Engine::allocateGlobalBuffersAndPrograms() {
  ...
  { // Scene buffer
    ...
    sceneBuffer->clearColor = glm::vec3{0.8, 1., 1.};
    sceneBuffer->clearAlpha = 1.0;
  }
  ...
  { // "Final" scene buffer (after resolving)
    sceneColorFinal = generateTextureBuffer(TextureFormat::RGBA16F, view::bufferWidth/2, view::bufferHeight);

    sceneBufferFinal = generateFrameBuffer(view::bufferWidth/2, view::bufferHeight);
    sceneBufferFinal->addColorBuffer(sceneColorFinal);
    sceneBufferFinal->setDrawBuffers();

    sceneBufferFinal->clearColor = glm::vec3{1., 0., 1.};
    sceneBufferFinal->clearAlpha = 1.0;
  }
  ...
  { // Alternate display buffer
    ...
    displayBufferAlt->clearColor = glm::vec3{1., 0.2, 1.};
    displayBufferAlt->clearAlpha = 1.0;
  }
