...
void GroundPlane::prepare() {
  ...
  if (options::groundPlaneMode == GroundPlaneMode::TileReflection ||
    ...
    sceneAltFrameBuffer->clearColor = glm::vec3{1., 1., 0.1};
    sceneAltFrameBuffer->clearAlpha = 1.0;
  }
  ...
  if (options::groundPlaneMode == GroundPlaneMode::ShadowOnly) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      blurFrameBuffers[i]->clearColor = glm::vec3{1., 0.1, 1.};
      blurFrameBuffers[i]->clearAlpha = 1.0;
    }
