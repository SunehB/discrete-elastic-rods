...
void renderScene() {
  ...
  if (render::engine->getTransparencyMode() == TransparencyMode::Pretty) {
    ...
    render::engine->sceneBufferFinal->clearColor = glm::vec3{1., 0., 0.};
    render::engine->sceneBufferFinal->clearAlpha = 1.;
...
void buildPolyscopeGui() {
  ...
  ImGui::SetNextWindowPos(ImVec2(view::windowWidth - leftWindowsWidth - imguiStackMargin, imguiStackMargin));
...
void buildStructureGui() {
  ...
  ImGui::SetNextWindowPos(ImVec2(view::windowWidth - leftWindowsWidth - imguiStackMargin, lastWindowHeightPolyscope + imguiStackMargin));
  ImGui::SetNextWindowSize(
      ImVec2(leftWindowsWidth, 0));
      // view::windowHeight - lastWindowHeightPolyscope - 3 * imguiStackMargin
...
void buildBezierGui() {
    // Create window
    static bool showBezierWindow = true;

    ImVec2 st_pos = ImVec2(view::windowWidth - leftWindowsWidth - imguiStackMargin, lastWindowHeightPolyscope + imguiStackMargin);
    ImGui::SetNextWindowPos(st_pos);
    ImGui::SetNextWindowSize(ImVec2(leftWindowsWidth, 0));
    ImGui::Begin("Draw Bezier Curve", &showBezierWindow);
    ImGui::Text("Bezier Curves");

    if (ImGui::Button("Reset View")) {
        std::cout << "Reset View" << std::endl;
    }
    if(ImGui::CollapsingHeader("Random Bezier Curves")) {
      static Beziermanager bm;

      if (ImGui::Button("Add Test Bezier Curve")) {
        bm.init();
        bm.generateCurve();
      }
      if(ImGui::Button("Show Random Vector on Nodes")){
          bm.gen_randomvec();
      }
      if(ImGui::Button("Tangent, Normal and Binormals on Edges")){
          bm.gen_binormal();
      }
      if(ImGui::Button("Show Fresnt Frame")){
          bm.get_bishopframe();
      }
      if(ImGui::Button("Minimize Twisting Energy")){
          std::cout << "Minimize Twisting Energy" << std::endl;
      }
    }

    if(ImGui::CollapsingHeader("Draw Bezier Curves")) {
      if (ImGui::Button("Generate Basic Bezier Curve")) {
          std::cout << "Add Bezier Curve" << std::endl;
      }
      static bool BezierEditMode = false;
      ImGui::Checkbox("Enable Edit Mode", &BezierEditMode);
      ImGui::Text("G for Move");
      ImGui::Text("R for Rotate");
      ImGui::Text("S for Scale");
      ImGui::Text("X for Delete");
      ImGui::Text("Alt-G for snap/merge two points");
      static int n = 2;
      ImGui::Text("number of vertices: %d",n);
    }
    ImGui::End();
}
